...
func _on_pressed() -> void:
	Signals.boardOrders_OpenMenu.emit()
...
func deleteOrderFromCesta(order):
	print("Llega la orden a cesta")
	orders.erase(order)
	delete_order_Tablero()

func updateTextureCesto():
	var cesto_TexturePath = "res://Sprites/Estaciones/Loop de Trabajo/Cesto/cubo animacion/cesto llenandose%s.png" % orders.size()
...
func delete_order_Tablero():
	...
	grid_container.get_child(0).free()
