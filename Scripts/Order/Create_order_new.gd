...
@onready var label_PriceOrder = $OrderTexture/OrderNumPanel3/total_money
@onready var label_moneyOrder = $OrderTexture/OrderNumPanel3/Money_to_win
@onready var label_statusOrder = $OrderTexture/OrderNumPanel2/StatusOrder

var dictionary_statusOrder: Dictionary = {
	"To Do": Color.RED,
	"Washing": Color.BLUE,
	"Drying": Color.GREEN,
	"Ironing": Color.ORANGE,
...
func set_order(new_order, slot_width, slot_height, imagePath):
	...
	label_PriceOrder.text = str(order.price) + "$"
	update_moneyOrder()
	
	update_status_order("To Do")
...
func update_moneyOrder():
	moneyOrder = order.update_money_Order()
	label_moneyOrder.text = str(moneyOrder) + "$"

	if order.price <= 0:
		return

	var percentage = clamp(moneyOrder / order.price, 0.0, 1.0)

	var red = Color.RED
	var green = Color.GREEN

	label_moneyOrder.modulate = red.lerp(green, percentage)
...
func update_status_order(newStatus):
	label_statusOrder.text = newStatus
	label_statusOrder.modulate = dictionary_statusOrder[newStatus]
